# /spec-tests-first:init Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement `/spec-tests-first:init` as a new pre-cycle skill for `spec-tests-first` v2.2 that bridges existing repositories into STF's structure via three auto-detected cases (A/B/C) per `docs/superpowers/specs/2026-05-11-sdd-init-design.md`.

**Architecture:** One new skill at `plugins/spec-tests-first/skills/init/` (SKILL.md + a references file). Minor `/spec-tests-first:run` update so the orchestrator detects an un-initialized repo and offers to run `/spec-tests-first:init` first. README + plugin.json bump to 2.2.0.
...
| `plugins/spec-tests-first/skills/run/SKILL.md` | Modify | Add detection for un-initialized repos at the start; offer to run `/spec-tests-first:init` |
| `plugins/spec-tests-first/README.md` | Modify | Document `/spec-tests-first:init` in the cycle diagram + "What changed in v2.2" section |
...
11. **Slice 11** — `/spec-tests-first:run` integration
...
This file lists per-profile glob patterns for `/spec-tests-first:init`'s codebase-map scaffolding step. Read it on demand for the profile(s) resolved in `CLAUDE.md ## Test layout`. Per-profile sections will be filled in by Slice 5.
...
git commit -m "feat(stf): scaffold /spec-tests-first:init skill directory + frontmatter shell"
```
...
git commit -m "feat(stf): /spec-tests-first:init auto-detection of Case A/B/C"
```
...
Same as `/spec-tests-first:build`'s pre-checks 2 + 3:
...
On (a): present the 12 built-in profiles + `custom`. Save to CLAUDE.md `## Test commands` + `## Test layout` (same format as `/spec-tests-first:build`'s Step 2/3). For multi-language plans, offer multi-service config now.
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init Case A (greenfield)"
```
...
Goal: shared profile-resolution path that Cases B and C both invoke. Mostly delegates to logic already in `/spec-tests-first:build`'s Step 2/3 but doesn't write spec-status.md (no specs yet for B; specs handled separately in C).
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init shared profile + CLAUDE.md seeding (Cases B, C)"
```
...
git commit -m "feat(stf): /spec-tests-first:init reference patterns for 12 built-in profiles + custom"
```
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init codebase-map seeding from reference patterns"
```
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init Case B (existing codebase, no specs)"
```
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init Case C catalog + restructure + US-N to AC-N.M conversion"
```
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init Case C — spec-status, two-signal scan, warnings"
```
...
```bash
git add plugins/spec-tests-first/skills/init/SKILL.md
git commit -m "feat(stf): /spec-tests-first:init idempotency rules + Red Flags + Rules"
```

---

## Slice 11 — `/spec-tests-first:run` integration

Goal: `/spec-tests-first:run` detects an un-init'd repo and offers to run `/spec-tests-first:init` before Phase 1.

### Task 11.1: Update `/spec-tests-first:run` to detect un-init'd state

**Files:**
- Modify: `plugins/spec-tests-first/skills/run/SKILL.md`

- [ ] **Step 1: Add a Phase 0 detection at the start of /spec-tests-first:run**

Insert a new section at the top of `/spec-tests-first:run`'s body, just before the "Resumability check (run before Phase 1)" section:
...
```
The v2 cycle is **six phases**: spec → build → review → fix → validate → ship. An optional **Phase 0 (/spec-tests-first:init)** runs before spec for un-initialized repos.
```
...
git commit -m "feat(stf): /spec-tests-first:run detects un-init'd repos and offers /spec-tests-first:init"
```
...
- [ ] **Step 1: Add /spec-tests-first:init to the cycle diagram**

Find the cycle diagram and prepend `/spec-tests-first:init`:
...
Expected: zero hits — /spec-tests-first:init describes itself as "Pre-cycle", not as "Phase 0 of the SDD cycle" (it's not part of the cycle per se, it's preparatory).

- [ ] **Step 5: Commit**

```bash
git add plugins/spec-tests-first/.claude-plugin/plugin.json plugins/spec-tests-first/README.md
git commit -m "chore(stf): bump plugin.json to v2.2.0 and document /spec-tests-first:init in README"
```
...
3. Run a grep for `/spec-tests-first:tests` or other deprecated terms where they should NOT appear.
