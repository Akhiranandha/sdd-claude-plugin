...
1. **Rigorous Test-Driven Development.** Replace the "batch tests-first" model with a **per-AC red-green-refactor** loop inside `/sdd:build`. Every test is watched failing before any production code is written; every fix is verified green before moving on.
2. **Code-review phase in the cycle.** A new `/sdd:review` phase dispatches the two reviewer agents from the `review-code` plugin (quality + security) in parallel against the feature's changed files and writes a timestamped report to disk.
3. **Interactive fix phase.** A new `/sdd:fix` phase walks findings one-by-one with revert-on-regression, mutates the report in-place with per-finding status, and gates progress on Critical findings.

The cycle goes from 5 phases (`spec → tests → build → validate → ship`) to **6 phases** (`spec → build → review → fix → validate → ship`); `/sdd:tests` is folded into `/sdd:build` because per-AC RGR means tests are written **during** implementation, not before it.

The plugin becomes **fully self-contained** — zero external plugin dependencies. The PR-level `/code-review` is dropped (redundant with `/sdd:review`) and `commit-commands` is inlined (so commit messages and PR bodies can be SDD-aware: AC-IDs, validation summary, review status).
...
- Re-doing `/sdd:spec` or `/sdd:validate`. Both work fine today and only get minor metadata-tracking tweaks.
...
```
/sdd:spec  → /sdd:build → /sdd:review → /sdd:fix → /sdd:validate → /sdd:ship
                (RGR)       (read)       (edit)       (acceptance)    (commit/PR)
```

| Phase | Role | Status in v2 |
|---|---|---|
| `/sdd:spec` | Interactive 8-section spec writing with AC-IDs and VS-IDs | unchanged |
| `/sdd:build` | Per-AC red-green-refactor: write 1 test → watch RED → minimal impl → watch GREEN → refactor → next AC. Owns framework detection, test scaffolding, `spec-status.md`, `codebase-map.md`. | **major rewrite** |
| `/sdd:review` | Dispatch `code-quality-reviewer` + `security-reviewer` in parallel → `code-reporter` writes `./reports/code-review_<feature>_<ts>.md`. Read-only. No verdict gating. | **new** |
| `/sdd:fix` | Read latest report → walk findings severity-then-file → per finding: ask `fix / custom / skip / defer` → apply edit → re-run AC tests → revert if any AC regresses → mark status in report → atomic commit per successful fix → optional re-run `/sdd:review` at the end (recommended). | **new** |
| `/sdd:validate` | Walk VS-N steps (auto + manual). Unchanged from today. | unchanged |
| `/sdd:ship` | Inline commit/push/PR + merge. SDD-aware commit message and PR body. | **simplified** |

Helpers (unchanged in role, updated for the new cycle):
- `/sdd:update <feature>` — amend spec, mark changed ACs `stale`, no test regeneration (done by `/sdd:build`'s RGR loop).
- `/sdd:run <feature>` — orchestrator chains the 6 phases with checkpoints.
- `/sdd:status [<feature>]` — read-only inspector, adds `Latest review:` and findings counts.
...
Reason for copying rather than depending: `/sdd:ship` historically declared external deps (`commit-commands`, `code-review`) and they were a friction point at install. Three small read-only agents are easy to keep in sync if `review-code` evolves; the alternative — adding a third install requirement — is a worse user experience.
...
|---|---|
| `commit-commands@claude-plugins-official` | Inlined in `/sdd:ship` — an SDD-aware commit message/PR body (AC-IDs, validation summary, review status) is more useful than a generic one, and inlining is ~40 lines of Bash. |
| `code-review@claude-plugins-official` | Redundant with `/sdd:review` (runs the same kind of checks, after the fact, with merge already imminent). |

### System binaries

- `git` — used by `/sdd:build` (stash, branch, atomic commits per fix), `/sdd:fix` (per-fix commits, revert), and `/sdd:ship` (commit, push, branch detection).
- `gh` (GitHub CLI), authenticated — used by `/sdd:ship` only when a remote is configured (push, PR, merge).
- Project test runners — resolved from `CLAUDE.md ## Test commands` (and `## Test layout`); plugin remains language-agnostic.

## Section 2 — `/sdd:build` per-AC RGR engine
...
- **Single commit at end of build** (if git in use): `git add -A && git commit -m "build(<feature>): implement spec — N ACs pass"`. Captures the full feature implementation as one commit on the feature branch. `/sdd:fix` then adds incremental commits on top; `/sdd:ship` only handles whatever (usually nothing) is uncommitted at ship time.
...
| Watch the RED happen | The subagent returns JSON; `/sdd:build` verifies, never trusts "it should fail". |
...
### Detection on first `/sdd:spec` in a repo
...
`/sdd:build` reads the service for each AC, looks up that service's profile in `CLAUDE.md`, writes the test in the right `tests_root`, and uses that service's test command.
...
## Section 4 — `/sdd:review` (new phase)
...
- **Status column on every finding, initialized to `pending`** (new — `/sdd:fix` mutates it)
...
Each finding gets a stable **`finding-id`** (e.g., `SEC-001`, `QUA-014`) embedded by the reporter, so `/sdd:fix` can address them by ID across multiple sessions without ambiguity.
...
Next: /sdd:fix <feature>   — walk findings one-by-one with revert-on-regression
   or /sdd:validate <feature> — proceed without fixing (you can /sdd:fix later)
```
...
The pointer is overwritten every time `/sdd:review` (or `/sdd:fix`'s optional re-review) produces a new report.

### Design invariants

- Read-only by tool grant. Only `code-reporter` has `Write`, and only for the report file.
- No verdict gating — `/sdd:review` always reports; gating lives in `/sdd:fix` and in `/sdd:run`.
- The report path goes into `spec-status.md` as a one-line `Latest review: <path>` so `/sdd:fix` can locate the most recent report deterministically.
- Every invocation produces a **fresh timestamped report**. Old reports stay on disk for history.

## Section 5 — `/sdd:fix` (new phase)
...
Findings already `fixed` or `skipped` are not re-prompted — `/sdd:fix` is resumable.
...
The reporter wrote a `Status` column initialized to `pending` for every finding. `/sdd:fix` updates each row as it goes — same file, no shadow copy.
...
Re-runs of `/sdd:fix` append more rows; never rewrite history.
...
### Step 7 — Re-run `/sdd:review`? (recommended)

```
> Re-run /sdd:review on the fixed code to confirm a clean state?
...
On (a): dispatch `/sdd:review` again, write a new timestamped report, update `spec-status.md`'s `Latest review:` pointer. If the new report has ANY Critical findings, return to `/sdd:fix` loop on the new report. Otherwise output next-step pointer to `/sdd:validate`.

On (b): output next-step pointer to `/sdd:validate`.
...
| Case | Behavior |
|---|---|
| No report exists | Stop. "Run `/sdd:review $1` first." |
| Suggested-fix references code that's already changed (drift) | Edit fails or applies incorrectly → mark `deferred`, note "code drifted; re-run `/sdd:review`". |
| Multiple reports exist | Always pick the newest. User can override: `/sdd:fix $1 --report <path>`. |
| Phase invoked during `fail` / `blocked` build state | Stop. "Finish `/sdd:build $1` first." |

### Resumability

The report is the single source of truth and is mutated in place. Calling `/sdd:fix $1` a second time picks up exactly where it left off. No state lives in memory between sessions.

## Section 6 — `/sdd:ship` (simplified)
...
1. `spec-status.md` shows every AC = `pass`.
2. **No `pending` or `deferred` Critical findings** in the latest review report. If found, stop with "address Critical findings in `/sdd:fix` first."
...
- ~~Inline pre-commit code review~~ → handled by `/sdd:review`.
- ~~`commit-commands:commit` / `commit-commands:commit-push-pr`~~ → inlined.
- ~~PR-level `/code-review`~~ → redundant with `/sdd:review`.

## Section 7 — Helpers (`/sdd:run`, `/sdd:update`, `/sdd:status`)

### `/sdd:run <feature>` — orchestrator
...
### `/sdd:update <feature>` — iteration

1. Snapshot `spec.md` → `spec.md.prev`.
2. Re-open spec for edits (same template as `/sdd:spec`).
...
4. Tell user: "Run `/sdd:build <feature>` — only `stale` and `not-started` ACs iterate via RGR."

`/sdd:update` does NOT regenerate tests itself. `/sdd:build`'s RGR loop handles it, scoped to the changed AC subset.

### `/sdd:status [<feature>]` — read-only inspector
...
**`skills/tests/SKILL.md`:** kept as a thin deprecation shim. The file's body is replaced with a short message that prints when invoked: "Folded into `/sdd:build`. Run `/sdd:build $1` directly — it now handles test scaffolding + per-AC RGR." No work performed; user is redirected. The shim can be removed entirely in a later release once existing scripts/runs are unlikely to invoke `/sdd:tests`.
...
├── reports/
│   └── code-review_<feature>_<ts>.md  (in-place-mutated by /sdd:fix)
...
| `docs/specs/<feature>/spec-status.md` | Read as-is; missing `Latest review:` field is fine (added next time `/sdd:review` runs). |
| `tests/<feature>/` directories | Continue to work for Python projects; first new `/sdd:build` may prompt to confirm profile. |
| `/sdd:tests` command invocation | Deprecation shim that prints: "Folded into `/sdd:build`. Run `/sdd:build $1` directly — it now handles test scaffolding + per-AC RGR." No work performed. |
| CLAUDE.md `## Test commands` single-block | Reads unchanged. Multi-service requires user to opt in via monorepo detection on first `/sdd:spec`. |
...
2. **TDD model: per-AC red-green-refactor.** Tests written inside `/sdd:build`, one at a time.
3. **Code-review placement: new phase between build and validate.**
4. **Ship's inline review: dropped.** Replaced by `/sdd:review`.
5. **Fix model: separate phase (`/sdd:fix`), interactive, severity-then-file ordering, status column in-place, revert-on-regression.**
6. **Re-run `/sdd:review` at end of `/sdd:fix`: yes, recommended.** Default to (a).
7. **Tests phase: dropped entirely.** Folded into `/sdd:build`.
...
2. **`/sdd:build` rewrite.** Biggest change. Implement RGR loop, profile system, monorepo handling, cluster-boundary checkpoints, per-AC cap.
3. **Drop `/sdd:tests`** and add deprecation shim.
...
5. **Add `/sdd:review` skill.**
6. **Add `/sdd:fix` skill.** Includes revert-on-regression, atomic-commit-per-fix, report mutation, Critical gate, optional re-review.
7. **Rewrite `/sdd:ship`.** Inline commit/push/PR/merge, drop external deps.
8. **Update `/sdd:run`, `/sdd:update`, `/sdd:status`.** Adjust for 6-phase cycle and new artifacts.
...
Each slice ends with `/sdd:run` (or its v2 equivalent) demonstrating the cycle still works on a known-good feature.
